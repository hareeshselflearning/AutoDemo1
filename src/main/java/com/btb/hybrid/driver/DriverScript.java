...
public class DriverScript {
	...
	public void executeKeywords(String testCaseName, Xls_Reader xls, Hashtable<String, String> testData) throws Exception{
		...
		app.assertAll();
			...
			test.log(Status.FAIL, "MisMatch in Data(Locator or Excel) : Data Read Failed. Exception Details: "+e.getCause());
		
		}
	}

	
	public void executeKeywordsWebVitals(String testCaseName, Xls_Reader xls, String sheetName, Hashtable<String, String> testData) throws Exception{
		try {
		int rowCount = xls.getRowCount(Constants.KEYWORDS_SHEET);
		//app = new AppKeywords();
		app = new WebGenericKeywords();
		...
		app.setData(testData);
		app.setExtentTest(test);
		app.setExcelData(xls);
		app.setSheetName(sheetName);
		for (int rNum=2;rNum<=rowCount;rNum++){
			String tcid = xls.getCellData(Constants.KEYWORDS_SHEET, Constants.TCID_COL, rNum);
			if(tcid.equalsIgnoreCase(testCaseName)){
				String descVal = null;
		...
		app.assertAll();
			...
			test.log(Status.FAIL, "MisMatch in Data(Locator or Excel) : Data Read Failed. Exception Details:--> "+e.getMessage());
